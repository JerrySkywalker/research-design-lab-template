...
$required = @(
    'AGENTS.md', 'README.md', 'CHANGELOG.md', 'CONTRIBUTING.md', 'SECURITY.md', '.gitignore', '.github/workflows/template-checks.yml', 'scripts/validate_template.py',
...
if ($failures.Count -eq 0) {
    ...
    $governance = Get-Content -LiteralPath (Join-Path $root 'docs/GOVERNANCE.md') -Raw
    foreach ($term in @('CAPTURE', 'ROUTINE_UPDATE', 'CANONICAL_CHANGE', 'Generated output', 'Evidence', 'Interpretation', 'Decision', 'nested `AGENTS.md`')) {
        if ($governance -notmatch [regex]::Escape($term)) { $failures.Add("Governance contract missing: $term") }
    }
